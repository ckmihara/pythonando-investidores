...
def validate_file_size(file):
    max_size_kb = 500  # Defina o tamanho máximo permitido em kilobytes (KB)
    if file.size > max_size_kb * 1024:
        raise ValidationError(f"O tamanho máximo permitido é {max_size_kb} KB.")
class Empresas(models.Model):
    ...
    pitch = models.FileField(upload_to='pitchs', validators=[validate_file_size])
    logo = models.FileField(upload_to='logo', validators=[validate_file_size])
    ...
    def clean(self):
        super().clean()
        validate_file_size(self.pitch)
        validate_file_size(self.logo)
